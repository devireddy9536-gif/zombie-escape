// ===============================
// script.js (updated - hint display fix)
// ===============================
...
let msgEl = null;              // general message area (used for penalty/response textual messages)
let hintEl = null;             // separate element for hint text
...
function ensureControls() {
  ...
  // responseTimerEl & hintEl placement
...
function ensureResponseTimerEl() {
  ...
  // append to parent of room title (the flex wrapper)
...
function startResponseTimer(seconds) {
  ...
  // keep hintEl untouched (hint has own element)
  if (!msgEl) createMsgEl();
...
function clearResponseTimer() {
  ...
  // hide the small visual timer
  if (responseTimerEl) {
    responseTimerEl.style.visibility = "hidden";
  }
}

// ===== Hint element (separate from msgEl) =====
function ensureHintEl() {
  ...
  hintEl.style.background = "rgba(0,0,0,0.25)";
  hintEl.style.color = "#9ad88e";
  hintEl.style.display = "none"; // hidden by default

  // append *above* msgEl (so hint appears visually above the penalty/response message)
  // we'll insert hintEl into choiceButtons container
  choiceButtons.appendChild(hintEl);
}

// show hint for room (only once per room)
function showHintForRoom(index) {
  ...
  // if already hinted for this room, show brief 'try again' message in msgEl and return
  ...
  // populate and show hintEl
  hintEl.innerHTML = `<strong>Hint:</strong> ${room.hint || "Think again — look for a keyword in the poem."}`;
  hintEl.style.display = "block";

  // remove the hint after a short period (5s) but do NOT disturb msgEl or responseTimerEl
  setTimeout(() => {
    if (hintEl) {
