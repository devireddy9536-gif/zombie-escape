// ===============================
// script.js — hearts lives (Option A)
// ===============================

// ===== CONFIG =====
const START_LIVES = 3;               // starting lives (hearts)
const PENALTY_SECONDS = 15;          // wait time after a wrong choice (changed to 15s)
...
// ===== Ensure Controls (lifeline button + hearts UI) =====
...
function applyFiftyFifty() {
  ...
  for (let i = 0; i < toHideCount; i++) wrongBtns[i].style.display = "none";

  lifelineUsed = true;
  if (lifelineBtn) { lifelineBtn.disabled = true; lifelineBtn.style.opacity = "0.6"; }

  if (!msgEl) createMsgEl();
  msgEl.style.color = "#9ad88e";
  msgEl.textContent = "Lifeline used: two wrong options removed.";
  setTimeout(() => { if (msgEl) { msgEl.remove(); msgEl = null; } }, 2000);
}
...
function updateLivesUI() {
  if (!livesEl) return;
  // create hearts string (solid red hearts for remaining lives)
  const heart = "❤️";
  const empty = ""; // no empty hearts per Option A
  let html = "";
  for (let i = 0; i < lives; i++) html += `<span class="heart">${heart}</span>`;
  // optionally show nothing when lives==0
...
function deductLife() {
  ...
  if (prev > lives && livesEl) {
    // animate last heart with a pulse then fade
